<div class="whitespace-nowrap">
    <template x-for="calendarLoopItem in calendars.filter(calendar => calendar.group === '{{ $group }}');">
        <div class="rounded-md p-1 bg-primary-500 text-white dark:text-white"
             x-bind:class="{'bg-primary-500 text-white': calendarLoopItem.id === calendarId}"
        >
            ...
            >
                ...
                <template x-if="calendarLoopItem.resourceEditable === true">
                    <div class="cursor-pointer flex items-center">
                        <div class="w-5 h-5" x-on:click="">
                            <svg x-on:click="$dispatch('edit-calendar', calendarLoopItem)" x-cloak x-show="calendarLoopItem.hover" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
    ...
    @if($group === 'my')
        <x-button icon="plus" class="w-full" x-on:click="$dispatch('edit-calendar', {})">
            {{ __('Create Calendar') }}
        </x-button>
    @endif
</div>
